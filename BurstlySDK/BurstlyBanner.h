...
// Initialize a new banner object for any zone id provided to you. Example:
//
// CGRect bannerFrame = CGRectMake(0, self.view.frame.size.height - BBANNER_SIZE_320x50.height, BBANNER_SIZE_320x50.width, BBANNER_SIZE_320x50.height);
// self.banner = [[[BurstlyBanner alloc] initWithAppId:YOUR_APP_ID zoneId:YOUR_ZONE_ID frame:bannerFrame anchor:kBurstlyAnchorBottom rootViewController:self delegate:self] autorelease];
//
// @param: anAppId - the SkyRocket appId for the application - See https://controls.skyrocketapp.com/Apps
// @param: zoneID - the SkyRocket zoneId for the banner zone
// @param: aFrame - the frame created using the banner sizes defined above and used to position the banner view within its super view
// @param: anAnchor - BurstlyAnchor value used to anchor the ad within the frame provided
// @param: aRootViewController - the root view controller used for modal display of banner landing pages
// @param: aDelegate - the BurstlyBannerDelegate object which will recieve messages about the status of the banner
